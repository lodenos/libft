...
extern isize _strlen(u8 const *restrict const source);
extern u8 *_strncat(u8 const *restrict const source,
                    u8 const *restrict destination, isize size);
extern i32 _strncmp(u8 const *restrict const sourceA,
                    u8 const *restrict const sourceB, isize size);
extern u8 *_strncpy(u8 const *restrict const source,
                    u8 const *restrict destination, isize size);
extern u8 *_strndup(u8 const *restrict const source, isize size);
extern u8 *_strndupa(u8 const *restrict const source, isize size);
